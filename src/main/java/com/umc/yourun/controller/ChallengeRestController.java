...
public class ChallengeRestController {
    ...
    @Operation(summary = "CHALLENGE_API_03 : 대기 중인 크루 챌린지 조회", description = "PENDING 상태인 크루 챌린지를 조회합니다.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "대기 상태 챌린지 조회 성공")
    })
    @GetMapping("/crew/pending")
    public ApiResponse<List<CrewChallenge>> retrieveChallenge() {
        List<CrewChallenge> pendingChallenges = challengeService.retrieveCrewChallenge();
        return ApiResponse.success("대기 중인 크루 챌린지를 조회합니다.", pendingChallenges);
    }
}
